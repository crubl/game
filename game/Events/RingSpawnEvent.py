...
class RingSpawnEvent(BaseEvent):
    ...
    def __init__(self, trigger_time: float, enemy_count: int, radius: int, enemy_class, 
                 speed_multiplier: float = 1.0, repeat: bool = False, interval: float = 0):
        """
        ...
        """
        super().__init__(trigger_time, repeat, interval)
    ...
    def execute(self, game_field):
        ...
        repeat_info = " (повторяется)" if self.repeat else ""
        print(f"[Ивент] Кольцевой спавн: {self.enemy_count} врагов, "
            f"радиус {self.radius}, скорость x{self.speed_multiplier}{repeat_info}")